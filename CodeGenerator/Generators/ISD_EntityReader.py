...
def ISD_EntityReader_cpp():
	...
	for basetype in hlp.base_types:
		...
		for type_impl in basetype.variants:
			...
			lines.append(f'		{{')
			lines.append(f'		reader_status status = read_small_block<ValueType::{type_name},{implementing_type}>(this->sstream, key, key_length, false, &(dest_variable) );')
			...
			lines.append(f'		reader_status status = read_small_block<ValueType::{type_name},{implementing_type}>(this->sstream, key, key_length, true, &(dest_variable.value()) );')
			...
			lines.append(f'		}}')
			lines.append(f'')
			lines.append(f'	// {type_name}: optional_value<std::vector<{implementing_type}>>' )
			lines.append(f'	template <> bool EntityReader::Read<optional_value<std::vector<{implementing_type}>>>( const char *key, const u8 key_length, optional_value<std::vector<{implementing_type}>> &dest_variable )')
			lines.append(f'		{{')
			lines.append(f'		dest_variable.set();')
			lines.append(f'		reader_status status = read_array<ValueType::{array_type_name},{implementing_type}>(this->sstream, key, key_length, true, &(dest_variable.value()), nullptr );')
			lines.append(f'		if( status == reader_status::success_empty )')
			lines.append(f'			dest_variable.clear();')
			lines.append(f'		return status != reader_status::fail;')
			lines.append(f'		}}')
			lines.append(f'')
			lines.append(f'	// {type_name}: indexed_array<{implementing_type}>' )
			lines.append(f'	template <> bool EntityReader::Read<indexed_array<{implementing_type}>>( const char *key, const u8 key_length, indexed_array<{implementing_type}> &dest_variable )')
			lines.append(f'		{{')
			...
			lines.append(f'		}}')
			lines.append(f'')
			lines.append(f'	// {type_name}: optional_value<indexed_array<{implementing_type}>>' )
			lines.append(f'	template <> bool EntityReader::Read<optional_value<indexed_array<{implementing_type}>>>( const char *key, const u8 key_length, optional_value<indexed_array<{implementing_type}>> &dest_variable )')
			lines.append(f'		{{')
			lines.append(f'		dest_variable.set();')
			lines.append(f'		reader_status status = read_array<ValueType::{array_type_name},{implementing_type}>(this->sstream, key, key_length, true, &(dest_variable.value().values()), &(dest_variable.value().index()) );')
			lines.append(f'		if( status == reader_status::success_empty )')
			lines.append(f'			dest_variable.clear();')
			lines.append(f'		return status != reader_status::fail;')
			lines.append(f'		}}')
