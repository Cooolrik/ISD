...
def ISD_EntityReader_h():
	...
	for basetype in hlp.base_types:
		...
		for type_impl in basetype.variants:
			...
			lines.append('            template <> bool Read<optional_value<std::vector<' + type_impl_name + '>>>( const char *key, const u8 key_length, optional_value<std::vector<' + type_impl_name + '>> &value );')
			lines.append('            template <> bool Read<indexed_array<' + type_impl_name + '>>( const char *key, const u8 key_length, indexed_array<' + type_impl_name + '> &value );')
			lines.append('            template <> bool Read<optional_value<indexed_array<' + type_impl_name + '>>>( const char *key, const u8 key_length, optional_value<indexed_array<' + type_impl_name + '>> &value );')
		lines.append('')
...
def ISD_EntityReader_cpp():
	...
	for basetype in hlp.base_types:
		...
		for type_impl in basetype.variants:
			...
			lines.append('	template <> bool EntityReader::Read<' + implementing_type + '>( const char *key, const u8 key_length, ' + implementing_type + ' &dest_variable )')
			lines.append('		{')
			lines.append('		reader_status status = read_small_block<ValueType::' + type_name + ',' + item_type + ',' + num_items_per_object + ',false>(')
			lines.append('			this->sstream, ')
			lines.append('			key, ')
			lines.append('			key_length, ')
			lines.append('			' + ptr_to_items + '(dest_variable)')
			lines.append('			);')
			lines.append('')
			lines.append('		return status != reader_status::fail;')
			lines.append('		}')
			lines.append('')
			lines.append('	// ' + type_name + ': optional_value<' + implementing_type + '>' )
			lines.append('	template <> bool EntityReader::Read<optional_value<' + implementing_type + '>>( const char *key, const u8 key_length, optional_value<' + implementing_type + '> &dest_variable )')
			lines.append('		{')
			lines.append('		dest_variable.set();')
			lines.append('')
			lines.append('		reader_status status = read_small_block<ValueType::' + type_name + ',' + item_type + ',' + num_items_per_object + ',true>(')
			lines.append('			this->sstream, ')
			lines.append('			key, ')
			lines.append('			key_length, ')
			lines.append('			' + ptr_to_items + '(dest_variable.value())')
			lines.append('			);')
			lines.append('')
			lines.append('		if( status == reader_status::fail )')
			lines.append('			return false;')
			lines.append('')
			lines.append('		if( status == reader_status::success_empty ) ')
			lines.append('			dest_variable.clear();')
			lines.append('		return true;')
			lines.append('		}')
			lines.append('')
			#lines.append('	// ' + type_name + ': std::vector<' + implementing_type + '>' )
			#lines.append('	template <> bool EntityReader::Read<std::vector<' + implementing_type + '>>( const char *key, const u8 key_length, std::vector<' + implementing_type + '> &src_variable )')
			#lines.append('		{')
			#lines.append('		return Read_array<ValueType::' + type_name + ',' + implementing_type + ',' + item_type + ',' + num_items_per_object + '>(')
			#lines.append('			this->sstream, ')
			#lines.append('			key, ')
			#lines.append('			key_length, ')
			...
			#lines.append('	template <> bool EntityReader::Read<optional_value<std::vector<' + implementing_type + '>>>( const char *key, const u8 key_length, optional_value<std::vector<' + implementing_type + '>> &opt_src_variable )')
			#lines.append('		{')
			...
			#lines.append('	template <> bool EntityReader::Read<indexed_array<' + implementing_type + '>>( const char *key, const u8 key_length, indexed_array<' + implementing_type + '> &src_variable )')
			#lines.append('		{')
			#lines.append('		return Read_array<ValueType::' + type_name + ',' + implementing_type + ',' + item_type + ',' + num_items_per_object + '>(')
			#lines.append('			this->sstream, ')
			#lines.append('			key, ')
			...
			#lines.append('	template <> bool EntityReader::Read< optional_value<indexed_array<' + implementing_type + '>>>( const char *key, const u8 key_length, optional_value<indexed_array<' + implementing_type + '>> &opt_src_variable )')
			#lines.append('		{')
			#lines.append('		const bool has_value = opt_src_variable.has_value();')
