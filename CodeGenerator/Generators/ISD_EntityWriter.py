...
def ISD_EntityWriter_h():
	...
	for basetype in hlp.base_types:
		...
		for type_impl in basetype.variants:
			...
			lines.append('            bool Write( const char *key, const uint8 key_length, const ' + type_impl_name + ' &value );')
			lines.append('            bool Write( const char *key, const uint8 key_length, const optional_value<' + type_impl_name + '> &value );')
		lines.append('')
	...
	for basetype in hlp.base_types:
		...
		for type_impl in basetype.variants:
			type_impl_name = type_impl.implementing_type
			lines.append('            bool Write( const char *key, const uint8 key_length, const std::vector<' + type_impl_name + '> &value );')
			lines.append('            bool Write( const char *key, const uint8 key_length, const optional_value<std::vector<' + type_impl_name + '>> &value );')
			lines.append('            bool Write( const char *key, const uint8 key_length, const indexed_array<' + type_impl_name + '> &value );')
			lines.append('            bool Write( const char *key, const uint8 key_length, const optional_value<indexed_array<' + type_impl_name + '>> &value );')
		lines.append('')
...
def ISD_EntityWriter_cpp():
	...
	for basetype in hlp.base_types:
		...
		# we have a special implementation for bool, since it does not store vectors the same way
		...
		for type_impl in basetype.variants:
			...
			lines.append('	bool EntityWriter::Write( const char *key, const uint8 key_length, const ' + implementing_type + ' &src_variable )')
			lines.append('		{')
			...
			lines.append('	bool EntityWriter::Write( const char *key, const uint8 key_length, const optional_value<' + implementing_type + '> &src_variable )')
			lines.append('		{')
			lines.append('		return write_small_block<ValueType::' + type_name + ',' + item_type + ',' + num_items_per_object + '>(')
			lines.append('			this->dstream, ')
			lines.append('			key, ')
			lines.append('			key_length, ')
			lines.append('			(src_variable.has_value()) ? ' + ptr_to_items + '(src_variable.value().second) : nullptr')
			lines.append('			);')
			lines.append('		}')
			lines.append('')
			lines.append('	// ' + type_name + ': std::vector<' + implementing_type + '>' )
			lines.append('	bool EntityWriter::Write( const char *key, const uint8 key_length, const std::vector<' + implementing_type + '> &src_variable )')
			lines.append('		{')
			lines.append('		return write_large_block_array<ValueType::' + type_name + ',' + implementing_type + ',' + item_type + ',' + num_items_per_object + '>(')
			lines.append('			this->dstream, ')
			lines.append('			key, ')
			lines.append('			key_length, ')
			lines.append('			true, // not an optional value')
			lines.append('			(src_variable.size() > 0) ? ' + ptr_to_items + '(src_variable.front()) : nullptr, // nullptr if size is zero')
			...
			lines.append('	bool EntityWriter::Write( const char *key, const uint8 key_length, const optional_value<std::vector<' + implementing_type + '>> &opt_src_variable )')
			lines.append('		{')
			...
			lines.append('			key, ')
			lines.append('			key_length, ')
			lines.append('			has_value, // not an optional value')
			lines.append('			(has_value && src_variable.size() > 0) ? ' + ptr_to_items + '(src_variable.front()) : nullptr, // nullptr if size is zero')
			...
			lines.append('	bool EntityWriter::Write( const char *key, const uint8 key_length, const indexed_array<' + implementing_type + '> &src_variable )')
			lines.append('		{')
			lines.append('		return write_large_block_array<ValueType::' + type_name + ',' + implementing_type + ',' + item_type + ',' + num_items_per_object + '>(')
			lines.append('			this->dstream, ')
			lines.append('			key, ')
			lines.append('			key_length, ')
			lines.append('			true, // not an optional value')
			lines.append('			(src_variable.values().size() > 0) ? ' + ptr_to_items + '(src_variable.values().front()) : nullptr, // nullptr if size is zero')
			lines.append('			(src_variable.values().size() * ' + num_items_per_object + '), // number of items' )
			lines.append('			(src_variable.index().size() > 0) ? src_variable.index().data() : nullptr, // nullptr if size is zero')
			lines.append('			(src_variable.index().size()) // size of index')
			lines.append('			);')
			lines.append('		}')
			lines.append('')
			lines.append('	// ' + type_name + ': optional_value<indexed_array<' + implementing_type + '>>' )
			lines.append('	bool EntityWriter::Write( const char *key, const uint8 key_length, const optional_value<indexed_array<' + implementing_type + '>> &opt_src_variable )')
			lines.append('		{')
			lines.append('		const bool has_value = opt_src_variable.has_value();')
			...
			lines.append('			key_length, ')
			lines.append('			has_value, // not an optional value')
			lines.append('			(has_value && src_variable.values().size() > 0) ? ' + ptr_to_items + '(src_variable.values().front()) : nullptr, // nullptr if size is zero')
			lines.append('			(has_value) ? (src_variable.values().size() * ' + num_items_per_object + ') : 0, // number of items' )
			lines.append('			(has_value && src_variable.index().size() > 0) ? src_variable.index().data() : nullptr, // nullptr if size is zero')
