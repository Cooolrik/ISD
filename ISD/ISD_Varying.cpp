// ISD Copyright (c) 2021,2022 Ulrik Lindahl
// Licensed under the MIT license https://github.com/Cooolrik/ISD/blob/main/LICENSE

#pragma once

#include "ISD_Varying.h"
...
namespace ISD
    ...
    bool Varying::Deinitialize()
        ...
        if( this->IsInitialized() )
            ...
            if( !success )
                {
                ISDErrorLog << "Error in call to dynamic_types::delete_type" << ISDErrorLogEnd;
                return false;
                }
    ...
    void Varying::MF::Clear( Varying &obj ) 
        ...
        ISDRuntimeCheck( success, Status::EUndefined, "Could not clear the dynamic type" );
        }
    ...
    void Varying::MF::DeepCopy( Varying &dest, const Varying *source )
        ...
        ISDRuntimeCheck( success, Status::EUndefined, "Could not clear Varying type" );
        ...
        ISDRuntimeCheck( success, Status::EUndefined, "Cannot set Varying type." );

        // copy the data from source to dest
        success = dynamic_types::copy( dest.type_m, dest.container_type_m, dest.data_m, source->data_m );
        ISDRuntimeCheck( success, Status::EUndefined, "Cannot copy Varying type data." );
        }
    ...
    bool Varying::MF::Write( const Varying &obj, EntityWriter &writer )
        ...
        if( !obj.IsInitialized() )
            {
            ISDErrorLog << "Cannot write uninitialized Varying to stream. Use optional_value template for optional Varying data." << ISDErrorLogEnd;
            return false;
            }

        // store the data type index
        if( !writer.Write( ISDKeyMacro("Type"), (u16)obj.type_m ) )
            return false;

        // store the container type index
        if( !writer.Write( ISDKeyMacro("ContainerType"), (u16)obj.container_type_m ) )
            return false;

        // store the data
        if( !dynamic_types::write( obj.type_m, obj.container_type_m, ISDKeyMacro( "Data" ), writer, obj.data_m ) )
            return false;
    ...
    bool Varying::MF::Read( Varying &obj, EntityReader &reader )
        ...
        if( !reader.Read( ISDKeyMacro("Type"), type_u16 ) )
            return false;

        // load the container type index
        if( !reader.Read( ISDKeyMacro("ContainerType"), container_type_u16 ) )
            return false;
        ...
        // store the data
        if( !dynamic_types::read( obj.type_m, obj.container_type_m, ISDKeyMacro( "Data" ), reader, obj.data_m ) )
            return false;
    ...
    bool Varying::MF::Validate( const Varying &obj, EntityValidator &validator )
        ...
        if( !obj.IsInitialized() )
            {
            ISDValidationError( ValidationError::NullNotAllowed )
    ...
    bool Varying::MF::SetType( Varying &obj, data_type_index dataType, container_type_index containerType ) 
        ...
        if( !success )
            {
            ISDErrorLog << "Error in call to Varying::Deinitialize" << ISDErrorLogEnd;
            return false;
            }
